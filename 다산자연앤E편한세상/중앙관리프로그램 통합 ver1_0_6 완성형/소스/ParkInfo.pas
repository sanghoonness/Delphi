...
type
  TfrmParkInfo = class(TForm)
    ...
    procedure qryParkInfoReserve5GetText(Sender: TField; var Text: string;
      DisplayText: Boolean);
    procedure edtAvailablePointChange(Sender: TObject);
  private
  ...
  try
    if qryParkInfo.RecordCount > 0 then
      ...
      begin
        ...
        LabelPoint.Caption := '('+IntToStr(edtAvailablePoint.Value div 60)+'시간)';
...
procedure TfrmParkInfo.edtAvailablePointChange(Sender: TObject);
begin
    LabelPoint.Caption := '('+IntToStr(edtAvailablePoint.Value div 60)+'시간)';
end;
