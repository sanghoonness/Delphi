    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ExceptLogging(inttostr(nUnitNo) + '번 차단기OPEN');
        end
        else if nCmd = 2 then
        begin
          idTC.IOHandler.WriteLnRFC('BAR_CLOSE_1', enUTF8);
          ExceptLogging(inttostr(nUnitNo) + '번 차단기CLOSE');
        end;
...
var
  ...
  const
    ENDCM = ' 컬럼추가 완료';
  begin
    //DB버전에 따라 필요한 컬럼이 없는 경우 자동으로 추가하는 기능
    try
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          SQL.Text := 'ALTER TABLE '+Parameters.ParamByName('v1').Value+' ADD  '+Parameters.ParamByName('v2').Value+' VARCHAR(50)';
          ExecSQL;
          ExceptLogging('현대통신포인트 컬럼 추가'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('블랙리스트 삭제 컬럼 추가'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('포인트조회Table 생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('포인트조회startdatetime 생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('포인트조회 PointType 생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('포인트조회PointUnit  생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('스마트빌리지 사전방문차량Table 생성'+ENDCM);
        end;
      end;
      //순찰패드 추가
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('순찰패드 Table 생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          ExecSQL;
          ExceptLogging('아이콘트롤스(홈넷) 사전등록 횟수 테이블 생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          SQL.Text := 'ALTER TABLE '+Parameters.ParamByName('v1').Value+' ADD  '+Parameters.ParamByName('v2').Value+' int default 0 ';
          ExecSQL;
          ExceptLogging('정기차량 ParkingMin  생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        SQL.Text := 'select count(*) from INFORMATION_SCHEMA.COLUMNS where table_name=:v1 and column_name=:v2';
        Parameters.ParamByName('v1').Value := 'IOSData';
        Parameters.ParamByName('v2').Value := 'OverParkingMin';
        Open;
        if Fields[0].Value = 0 then begin
          Close;
          SQL.Text := 'ALTER TABLE '+Parameters.ParamByName('v1').Value+' ADD  '+Parameters.ParamByName('v2').Value+' int default 0 ';
          ExecSQL;
          ExceptLogging('정기차량 OverParkingMin  생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        if Fields[0].Value = 0 then begin
          ...
          SQL.Text := 'ALTER TABLE '+Parameters.ParamByName('v1').Value+' ADD  '+Parameters.ParamByName('v2').Value+' int default 0 ';
          ExecSQL;
          ExceptLogging('일반차량 OverParkingMin  생성'+ENDCM);
        end;
      end;
      ...
      with qryMainTemp do begin
        ...
        SQL.Text := 'select count(*) from INFORMATION_SCHEMA.COLUMNS where table_name=:v1 and column_name=:v2';
        Parameters.ParamByName('v1').Value := 'GGroup';
        Parameters.ParamByName('v2').Value := 'WP';
        Open;
        if Fields[0].Value = 0 then begin
          Close;
          SQL.Text := 'ALTER TABLE '+Parameters.ParamByName('v1').Value+' ADD  '+Parameters.ParamByName('v2').Value+' int default 0 ';
          ExecSQL;
          ExceptLogging('정기차량 WP  생성'+ENDCM);
        end;
      end;
...
begin
  try
    ...
    adminManager := aString(iSetup.ReadString('PARKING', '관리자ID', '(주)넥스파시스템'));
    ...
    nBlackUse   := iSetup.ReadInteger('PARKING', '블랙리스트', 0);
    nYogumMode  := iSetup.ReadInteger('PARKING', '요금세팅', 0);    //0:출입모니터링 현장, 1:요금계산기 현장
    nCurrUnitNo := iSetup.ReadInteger('PARKING', 'UnitNo', 0);
    nVisitReason   := iSetup.ReadInteger('PARKING', '방문사유', 0);
    nBujaeUse      := iSetup.ReadInteger('PARKING', '부제사용', 0);
    bBlockLPRUse   := iSetup.ReadBool('PARKING', 'LPR별진입차단사용', False);
    bIMageUse   := iSetup.ReadInteger('PARKING', '이미지처리', 1);
    PDAUSE   := iSetup.ReadInteger('PARKING', '순찰패드', 0);
    homenetuse   := iSetup.ReadInteger('PARKING', '홈넷', 0);
    nvisituse   := iSetup.ReadInteger('PARKING', '사전방문', 0);
    DelCustinfoAll := iSetup.ReadInteger('PARKING', '정기차량선택삭제', 0);
    smartVillagePoint := iSetup.ReadInteger('PARKING', '포인트동기화', 0);
    ...
    nWP  := iSetup.ReadInteger('PARKING', '정기차량사용시간대별', 0);
    if nBlackUse = 1 then                                //블랙리스트 사용시 헤당메뉴 표출
    ...
    begin
      ...
      mnuWP.Visible     := True;
      N32.Visible       := True; //무료개방시간설정
      N10.Visible       := True; //일별정산내역
      N11.Visible       := True; //월별정산내역
      N12.Visible       := True; //정산처리내역
      N19.Visible       := True; //주차시간별 내역
      N17.Visible       := True; //미인식 입차차량 차량번호 수정
      N20.Visible       := True; //개방시간 출입내역
      N43.Visible := True; //조건별 입차(출차,정산)현황
    ...
    begin
      try
        ...
        CheckDataBaseColumn; //DB버전 업데이트 따른 컬럼 자동추가
      except
        on E: Exception do
        begin
...
//           N8.Enabled:= True;
    ...
    begin
      ...
      if nHomenet_visit = 0 then //사전방문조회
      begin
        N46.Visible := False;
      end;

      if nvisituse = 0 then   //사전방문 등록 횟수(아이콘트롤스- 홈넷)
      begin
        N47.Visible := False;
      end;

      if nWP = 1 then  //정기차량 사용 시간대 설정
      begin
          mnuWP.Visible := True;
          N48.Visible := True;
      end;
...
var
  sReason: aString;
begin
  sReason:= InputBox('수동닫힘사유', '차단기수동닫힘사유를 입력하여주세요!', '');
