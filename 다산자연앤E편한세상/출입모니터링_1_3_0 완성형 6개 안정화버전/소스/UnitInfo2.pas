...
begin
  if (Integer(cmbUnitKind.Items.Objects[cmbUnitKind.ItemIndex]) = 3) or
    ...
    inputIpStr := InputBox('데이터복제','복제된 장비의 IP입력를 입력하세요(빈칸이면 동일하게 복제)',trim(edtIPNo.Text));
  end;
...
begin
  ...
  if Integer(cmbUnitKind.Items.Objects[cmbUnitKind.ItemIndex]) = 101 then begin
    ...
    edtMainLprNo.Color := clWhite;
    cmbUnitKind.Tag := 102;
  end else if Integer(cmbUnitKind.Items.Objects[cmbUnitKind.ItemIndex]) = 103 then begin
    edtMainLprNo.Enabled := True;
    edtMainLprNo.Color := clWhite;
    cmbUnitKind.Tag := 103;
  end else begin
...
begin
  try
    if MessageDlg('선택한 기기를 삭제할까요?', mtConfirmation, [mbYes, mbNo], 0) <> mrYes then Exit;
