...
uses
  ...
  AdvCombo, Vcl.ExtCtrls, AdvPanel, AdvOfficeButtons;
...
type
  TfrmSetup = class(TForm)
    ...
    Bevel29: TBevel;
    Label28: TLabel;
    edtHomeInfoIP: TAdvMaskEdit;
    edtHomeInfoPort: TAdvSpinEdit;
    Bevel30: TBevel;
    Label29: TLabel;
    Bevel31: TBevel;
    Label30: TLabel;
    edtEZVilleDong: TAdvEdit;
    edtEZVilleHo: TAdvEdit;
    cbMiIn: TAdvOfficeCheckBox;
    cb6Proc: TAdvOfficeCheckBox;
...
begin
  try
    ...
    iSetup.WriteString('PARKING', 'HomeInfo IP', MG_StrTrim(edtHomeInfoIP.Text, ' '));
    iSetup.WriteInteger('PARKING', 'HomeInfo Port', edtHomeInfoPort.Value);
    iSetup.WriteString('PARKING', 'ezVilleDong', edtEZVilleDong.Text);
    iSetup.WriteString('PARKING', 'ezVilleHo', edtEZVilleHo.Text);
    iSetup.WriteBool('PARKING', 'MI_IN', cbMiIn.Checked);
    iSetup.WriteBool('PARKING', '6Proc', cb6Proc.Checked);
...
begin
  try
    ...
    edtHomeInfoIP.Text         := MG_StrTrim(iSetup.ReadString('PARKING', 'HomeInfo IP', '10.10.10.10'), ' ');
    edtHomeInfoPort.Value      := iSetup.ReadInteger('PARKING', 'HomeInfo Port', 25008);
    edtEZVilleDong.Text        := iSetup.ReadString('PARKING', 'ezVilleDong', '100');
    edtEZVilleHo.Text          := iSetup.ReadString('PARKING', 'ezVilleHo', '900');
    cbMiIn.Checked             := iSetup.ReadBool('PARKING', 'MI_IN', False);
    cb6Proc.Checked            := iSetup.ReadBool('PARKING', '6Proc', False);
  except
