...
begin
  ...
  if AptnerMonitorUse = 1 then begin
    ExceptLogging('입차차량번호비교 : '+ nInCarNo1 +' '+ dgNomalCar.Cells[3, dgNomalCar.Row]);
    ExceptLogging('출차차량번호비교 : '+ nOutCarNo1+' '+ dgNomalCar.Cells[6, dgNomalCar.Row]);
    //입차전송
...
begin
   ...
   ExceptLogging('입차차량번호 : '+ nInCarNo1);
   ExceptLogging('출차차량번호 : '+ nOutCarNo1);
   ...
   begin
      ...
      ExceptLogging('입차 File 없음: ' + dgNomalCar.Cells[10,dgNomalCar.row]);
   end;
   ...
   begin
      ...
      ExceptLogging('출차 File 없음: ' + dgNomalCar.Cells[11,dgNomalCar.row]);
   end;
end;
...
begin
  ...
  if AptnerMonitorUse = 1 then begin
      ...
      HomeInfoLogging('일반차량 차량 입출 내역(수정) 아파트너 사용준비 완료');
  end;
end;
