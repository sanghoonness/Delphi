...
begin
  try
    if cmbManager.Text <> '시스템관리자' then
    begin
      with qryManager do
      begin
        Close;
...
begin
  try
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('Select * from Manager where ParkNo = :N1 and Reserve1 = :N2 Order By MNo');
      Parameters.ParamByName('N1').Value:= nCurrParkNo;
      Parameters.ParamByName('N2').Value:= nCurrUnitNo;    // Modified by LJH 2020-02-06 18:40:15 해당 기기에서만 ID뜨게해달라고 징징 대전성모병원
      Open;
  ...
  try
    ...
    begin
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('Select * from Manager where ParkNo = :pParkNo and MNo = :pMNo and Passwd = :pPasswd');
