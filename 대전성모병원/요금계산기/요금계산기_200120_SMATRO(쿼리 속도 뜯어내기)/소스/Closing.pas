...
begin
  try
    ...
    begin
      ...
      sRPrt:= sRPrt + '========================================' + LF + LF + CR;
      nCSeq:= iSetup.ReadInteger('PARKING', 'CLOSING_'+FormatDateTime('YYYY-MM-DD', Now), 0)+1;
      iSetup.WriteInteger('PARKING', 'CLOSING_'+FormatDateTime('YYYY-MM-DD', Now), nCSeq);
    end;

    sRPrt:= sRPrt + '근무시작: ' + sGeunmuStart + LF + CR;
    ...
    if nClosingType = 0 then
      sCDate:= FormatDateTime('YYYY-MM-DD', edtPDate.Date)  //일자마감
    else
      sCDate:= Copy(sGeunmuStart, 1, 10); //근무자마감
    ...
    begin
      ...
      begin
        ...
        Close;
        SQL.Clear;
        SQL.Add('Select GT1 from Gracetime where ParkNo = :N1');
        Parameters.ParamByName('N1').Value:= nCurrParkNo;
        Open;

        if RecordCount > 0 then
        begin
          sHour:= Copy(MG_StrTrim(FieldByName('GT1').AsString, ':'), 1, 2);
          sMin := Copy(MG_StrTrim(FieldByName('GT1').AsString, ':'), 3, 2);
          nGT1:= (StrToInt(sHour) * 60) + StrToInt(sMin);
        end
        else
          nGT1:= 0;
        ...
        if RecordCount > 0 then
        begin
          sDCProcTime:= FieldByName('ProcTime').AsString;
        end;
        ...
        if nClosingType = 0 then
          ...
          SQL.Add('where ProcDate >= :N1 and ChkClosing = :N2 and UnitNo = :N3 and ');
//        SQL.Add('(TKType = :N4 or TKType = :N5) and MNo = :N6 ');
        SQL.Add('(TKType = :N4 or TKType = :N5) and MNo = :N6 and Reserve2 <> :N7');
        Parameters.ParamByName('N1').Value:= sCDate;
        Parameters.ParamByName('N2').Value:= 0;
        Parameters.ParamByName('N3').Value:= nCurrUnitNo;
...
//          SQL.Add('(TKType = :N4 or TKType = :N5) and MNo = :N6 and  ');
          ...
          Parameters.ParamByName('N7').Value:= '무료시간출차';
          Open;
          edtRealCnt.Text:= IntToStr(FieldByName('nRealCnt').AsInteger);
          ...
          if RecordCount > 0 then
          begin
            edtLostCnt.Text:= FieldByName('COLUMN1').AsString;
            edtLostAmt.Text:= FieldByName('COLUMN2').AsString;
          end;
          Close;
          SQL.Clear;
...
//          SQL.Add('(TKType = :N4 or TKType = :N5) and UnitNo = :N6 and MNo = :N7 ');
          ...
          if RecordCount > 0 then
            edtTotDCCnt.Text:= FieldByName('COLUMN1').AsString
          else
            edtTotDCCnt.Text:= '0';
          //무료통과(GT1)건수
          if nGT1 > 0 then
          begin
            ...
            Parameters.ParamByName('N1').Value:= sCDate;
            Parameters.ParamByName('N2').Value:= 0;
            Parameters.ParamByName('N3').Value:= nGT1;
            Parameters.ParamByName('N4').Value:= 1;
            Parameters.ParamByName('N5').Value:= 9;
            Parameters.ParamByName('N6').Value:= nCurrUnitNo;
            Parameters.ParamByName('N7').Value:= nCurrMNo;
            ...
            if RecordCount > 0 then
              edtGT1Cnt.Text:= IntToStr(RecordCount)
            else
              edtGT1Cnt.Text:= '0';
          end
          else
          ...
          if nClosingType = 0 then
            SQL.Add('Select * from TKProc where ProcDate = :N1 and ChkClosing = :N2 ')
          else
            SQL.Add('Select * from TKProc where ProcDate >= :N1 and ChkClosing = :N2 ');

          SQL.Add(' and (TKType = :N4 or TKType = :N5) and UnitNo = :N6 and MNo = :N7 ');
          SQL.Add(' and Reserve2 = :N8' );
          Parameters.ParamByName('N1').Value:= sCDate;
          Parameters.ParamByName('N2').Value:= 0;
          Parameters.ParamByName('N4').Value:= 1;
          ...
          if RecordCount > 0 then
            edtGT2Cnt.Text:= IntToStr(RecordCount)
          else
            edtGT2Cnt.Text:= '0';
          ...
          if RecordCount > 0 then
            edtDCCnt.Text:= IntToStr(RecordCount)
          else
            edtDCCnt.Text:= '0';
//          ShowMessage('10');
          if nSelect = 2 then
        ...
        begin
          ...
          nBalTot:= FieldbyName('COLUMN2').AsInteger;
      ...
      begin
        ...
        begin
          ...
          sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
          sRPrt:= sRPrt + '개방건수: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtGT2Cnt.Text,',',''), 0)), 10) + '건' + LF + CR;
          sRPrt:= sRPrt + '처리건수: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtCnt.Text,',',''), 0)), 10) + '건' + LF + CR;

          sRPrt:= sRPrt + '수동출차: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtLostCnt.Text,',',''), 0)), 10) + '건  ' +
                                         MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtLostAmt.Text,',',''), 0)), 10) + '원' + LF + CR;
          sRPrt:= sRPrt + '주차금액: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtTot.Text,',',''), 0)), 10) + '원' + LF + CR;
          sRPrt:= sRPrt + '할인처리: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtDCCnt.Text,',',''), 0)), 10) + '건  ' +
                                         MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtDC.Text,',',''), 0)), 10) + '원' + LF + CR;
          sRPrt:= sRPrt + '총할인건: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtTotDCCnt.Text,',',''), 0)), 10) + '건' + LF + CR;
          sRPrt:= sRPrt + '유료건수: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtRealCnt.Text,',',''), 0)), 10) + '건' + LF + CR;
          sRPrt:= sRPrt + '수입금액: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtReal.Text,',',''), 0)), 10) + '건' + LF + CR;
          sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
...
//          sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('Select a.DCNo, a.DCName, count(*) as DCCnt, Sum(c.Recvamt) as DCRecvAmt, Sum(b.RealDCAmt) as DCRealDCAMT ');
            sql.Add(' from DCInfo as a ');
            sql.Add(' left outer join DCDetail as b ');
            sql.Add(' on a.DCNo = b.DCNo ');
            sql.Add(' left outer join TKProc as c ');
            sql.Add(' on b.TKNo = c.TKNo ');
            if nClosingType = 0 then
              sql.Add(' where c.ProcDate = :N1 and c.MNo = :N2 and c.UnitNo = :N3 and c.ChkClosing = :N4 ')
            else
             sql.Add(' where c.ProcDate >= :N1 and c.MNo = :N2 and c.UnitNo = :N3 and c.ChkClosing = :N4 ');
            sql.Add(' group by a.DCNo,a.DCName ');
            sql.Add(' order by a.DCNo asc ');

            Parameters.ParamByName('N1').Value:= sCDate;
            Parameters.ParamByName('N2').Value:= nCurrMNo;
            Parameters.ParamByName('N3').Value:= nCurrUnitNo;
            Parameters.ParamByName('N4').Value:= 0;
            ExceptLogging(sql.Text);
            commandtimeout := 1800;
            Open;
            ExceptLogging('Open3');
            ...
            begin
              ...
              begin
                sRPrt:= sRPrt + MG_Left(FieldByName('DCName').AsString, 20) +
                        ...
                        MG_Right(FormatFloat('#,##0', FieldByName('DCRecvAmt').AsInteger), 10) + '원' + LF + CR;
                Next;
              end;
            end;
            sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
            Close;
            SQL.Clear;
            SQL.Add('Select Count(*) as NormalCnt, sum(TotFee) as NormalFee from TKProc  ');
            ...
            SQL.Add(' (TotFee=0 or (TotFee>0 and TotFee=RecvAmt))  ');
            Parameters.ParamByName('N1').Value:= sCDate;
            Parameters.ParamByName('N2').Value:= 0;
            Parameters.ParamByName('N3').Value:= nCurrMNo;
            Parameters.ParamByName('N4').Value:= nCurrUnitNo;
            ...
            begin
              while not Eof do
              begin
                sRPrt:= sRPrt + '[일반차량]' + LF + CR;
                sRPrt:= sRPrt + MG_Right(FormatFloat('#,##0', FieldByName('NormalCnt').AsInteger), 25) + '건 ' +
                        MG_Right(FormatFloat('#,##0', FieldByName('NormalFee').AsInteger), 10) + '원' + LF + CR;
                Next;
              end;
              sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
            end;
      ...
      begin
        ...
        begin
          ...
          while not Eof do
          begin
            sRPrt:= sRPrt + MG_Right(IntToStr(FieldByName('UnitNo').AsInteger), 4) +
                            MG_Right(IntToStr(FieldByName('MNo').AsInteger), 8) +
