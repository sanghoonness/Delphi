...
begin
  try
    ...
    begin
      ...
      for j:= 1 to nType do
      begin
        // 반복문으로 콤보박스 인덱스값 가져와서 string형 변수에 저장
...
begin
  try
    ...
    for i:= 1 to 1 do
    begin
      // 윗줄문구, 아랫줄문구에 ini파일에서 읽어온 전광판 문구 넣기.
      TAdvEdit(FindComponent('edt' + IntToStr(i) + '_1')).Text:= MG_StrTrim(iSetup.ReadString('PARKING', 'DSP_' + IntToStr(i) + '_1', ''), AnsiChar(39));
      TAdvEdit(FindComponent('edt' + IntToStr(i) + '_2')).Text:= MG_StrTrim(iSetup.ReadString('PARKING', 'DSP_' + IntToStr(i) + '_2', ''), AnsiChar(39));
      // 윗줄문구 색상, 아랫줄문구 색상 ini파일에서 읽어와서 변수에 저장
      sColor1:= iSetup.ReadString('PARKING', 'DSP_COLOR_' + IntToStr(i) + '_1', '');
      sColor2:= iSetup.ReadString('PARKING', 'DSP_COLOR_' + IntToStr(i) + '_2', '');

      // ini파일에서 읽어온 윗줄문구 색상값 길이가 12이면
      if Length(sColor1) = nType then
      begin
        // 반복문으로 sColor1값 한자리씩 잘라서 콤보박스 인덱스값에 넣기
        for j:= 1 to nType do
