...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Inc(nErrCnt);
            sErrorLog:= sErrorLog + MG_Left(sCarNo, 12) + '-개별점에서 등록된 차량' + #13#10;
          end;
        end
        else
        ...
        begin
          ...
          begin
            //점별 쟈스민데이터에 있으면 개별점데이터인지 서버데이터인지 확인...
            //서버데이터이면 업데이트하고 개별점 데이터이면 업데이트안함...
    ...
    begin
      ...
      ExceptLogging('TfrmMain.TxtToDB: ' + sCarNo + ' 처리시 ' + aString(E.Message));
...
begin
    ...
    begin
      ...
      SQL.Add('Select Count(*) nIn from IONData where ProcDate = :N1 and Status = :N2');               // 미출차된것도포함
      Parameters.ParamByName('N1').Value:= FormatDateTime('YYYY-MM-DD', Now);
      Parameters.ParamByName('N2').Value:= 1;
...
begin
  try
    ...
    begin
      ...
      Parameters.ParamByName('N1').Value:= FormatDateTime('YYYY-MM-DD', Now);
      Parameters.ParamByName('N2').Value:= '수동출차';
      Parameters.ParamByName('N3').Value:= 0;
      Open;
  ...
  try
    ...
    begin
      ...
      Try
        ...
        begin
          iSetup.WriteString('PARKING', 'FTP File', sFileTime);      // FTP File -> 마지막 업로드(?) 날짜
      ...
      ExceptLogging('jasmine_car 파일 없음');
  except
...
begin
  ...
  begin
    ColWidths[0] := 30;
    ColWidths[1] := 70;   Alignments[1, 0] := taCenter;   // 부스번호
    ColWidths[2] := 150;  Alignments[2, 0] := taCenter;   // 부스명
    ColWidths[3] := 100;  Alignments[3, 0] := taCenter;   // 장비명
    ColWidths[4] := 200;  Alignments[4, 0] := taCenter;   // 장비에러
    ColWidths[5] := 100;  Alignments[5, 0] := taCenter;   // 발생일자
    ColWidths[6] := 100;  Alignments[6, 0] := taCenter;   // 발생시간
  end;
end;
