...
uses
  ...
  AdvDateTimePicker, ExtCtrls, tmsAdvGridExcel, AdvObj;
...
begin
  ...
  begin
    ...
    begin
      ...
      Close;
      SQL.Clear;
      SQL.Add('Select Count(*) nCnt from TKProc ');
      SQL.Add('where ProcDate >= :N1 and ProcDate <= :N2 and ParkingMin >= :N3 and ParkingMin <= :N4 and RealFee > :N5 ');
      Parameters.ParamByName('N1').Value:= sSDate;
      Parameters.ParamByName('N2').Value:= sEDate;
      ...
      sgCount.Cells[i, 3]:= FormatFloat('#,##0', FieldByName('nCnt').AsInteger);
      sgCount.Alignments[i, 3]:= taRightJustify;
      ...
      sgCount.Cells[i, 4]:= FormatFloat('#,##0', FieldByName('nCnt').AsInteger);
      sgCount.Alignments[i, 4]:= taRightJustify;
    end;
    ...
    SQL.Clear;
    SQL.Add('Select Count(*) nCnt from TKProc ');
    SQL.Add('where ProcDate >= :N1 and ProcDate <= :N2 and ParkingMin > :N3 and RealFee > :N4 ');
    Parameters.ParamByName('N1').Value:= sSDate;
    Parameters.ParamByName('N2').Value:= sEDate;
    Parameters.ParamByName('N3').Value:= 600;
