    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            ExceptLogging('HTTP 형식, 일반차량 입차 파일 확인: ' + sInFile);
          ...
          begin
            ExceptLogging('IP 형식, 일반차량 입차 파일 확인: ' + sInFile);
            if nLprRecog1 = 1 then
            begin
              sInFile:= FieldByName('InImage1').AsString;
            end
        ...
        begin
          ...
          ExceptLogging('일반차량 출차 파일 확인: ' + sOutFile);

          try
            // sFile1에 http가 붙을때
            if Copy(sOutFile, 1, 4) = 'http' then
            begin
              ExceptLogging('HTTP 형식, 일반차량 출차 파일 확인: ' + sOutFile);
            ...
            begin
              ExceptLogging('IP 형식, 일반차량 출차 파일 확인: ' + sOutFile);
              if nOutRecog1 = 1 then
              begin
                sOutFile:= FieldByName('OutImage1').AsString;
              end
