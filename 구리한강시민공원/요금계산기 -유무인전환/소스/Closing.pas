...
begin
  try
    ...
    nTaxTotal:= 0;
    nTaxResult:= 0;
    nMoneyTotal:= 0;    nCardTotal := 0;
    nMoneyCount:= 0;    nCardCount := 0;
    nTaxTotal :=0;
    nTax :=  0;
    nTaxResult := 0;
    sRPrt:= '';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('Select UnitNo, Count(*) cnt, Sum(RealFee) fee ');
            ...
            Parameters.ParamByName('N2').Value:= 0;
            Parameters.ParamByName('N3').Value:= nCurrUnitNo;
            Parameters.ParamByName('N4').Value:= 1;    // 현금
            Parameters.ParamByName('N5').Value:= nCurrMNo;
            Open;

            if RecordCount > 0 then
            begin
              nMoneyCount := FieldByName('cnt').AsInteger;   // 총 현금건수
              nMoneyTotal := FieldByName('fee').AsInteger;   // 총 현금
            end;
            ...
            Parameters.ParamByName('N2').Value:= 0;
            Parameters.ParamByName('N3').Value:= nCurrUnitNo;
            Parameters.ParamByName('N4').Value:= 2;   // 신용카드
            Parameters.ParamByName('N5').Value:= nCurrMNo;
            Open;

            if RecordCount > 0 then
            begin
              nCardCount := FieldByName('cnt').AsInteger;   // 총 카드건수
              nCardTotal := FieldByName('fee').AsInteger;   // 총 카드금액
            end;

            ExceptLogging('마감 > 현금건수: ' + IntToStr(nMoneyCount) + '건 / 현금액: ' + IntToStr(nMoneyTotal) + '원 / 카드건수: '
                      + IntToStr(nCardCount) + '건 / 카드금액: ' + IntToStr(nCardTotal) + '원');
          end;
          if nSelect = 2 then
          begin
            Close;  //마감일지에 추가
            ...
            SQL.Add('UnpaidCnt, Unpaid,Reserve1,Reserve2,Reserve3,Reserve4) ');
            SQL.Add('Values (:N1, :N2, :N3, :N4, :N5, :N6, :N7, :N8, :N9, :N10, ');
            SQL.Add(':N11, :N12, :N13, :N14, :N15, :N16, :N17, :N18, :N19, :N20, :N21, :N22, :N23, :N24) ');
            Parameters.ParamByName('N1') .Value:= nCurrParkNo;
            ...
            Parameters.ParamByName('N21').Value:= nMoneyCount;
            Parameters.ParamByName('N22').Value:= nMoneyTotal;
            Parameters.ParamByName('N23').Value:= nCardCount;
            Parameters.ParamByName('N24').Value:= nCardTotal;
            ExecSQL;
      ...
      begin
        ...
        begin
          ...
          sRPrt:= sRPrt + '주차금액: ' + FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtTot.Text,',',''), 0)) + '원' + LF + CR;
          ...
          sRPrt:= sRPrt + '수입금액: ' + MG_Right(FormatFloat('#,##0', StrToIntDef(MG_ReplaceStr(edtReal.Text,',',''), 0)), 10) + '원' + LF + CR;
          sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
...
//              if RecordCount > 0 then
//              begin
//                nMoneyTotal :=  FieldByName('COLUMN2').AsInteger;
//                nCreditCount:=  FieldByName('COLUMN1').AsInteger;
                sRPrt:= sRPrt + '신용카드: ' +
                        MG_Right(FormatFloat('#,##0',nCardCount ), 10) + '건 ' +
                        MG_Right(FormatFloat('#,##0',nCardTotal ), 10) + '원' + LF + CR;
...
//            begin
              sRPrt:= sRPrt + '현금합계: ' +
                      MG_Right(FormatFloat('#,##0',nMoneyCount ), 10) + '건 ' +
                      MG_Right(FormatFloat('#,##0',nMoneyTotal ), 10) + '원' + LF + CR;
...
//            end;
      ...
      if bCredit then
      begin
//         nTaxTotal :=(nMoneyTotal * 10) div 11;
//         nTax :=  nMoneyTotal - nTaxTotal;
//         nTaxResult := nMoneyTotal - nTax;
         sRPrt:= sRPrt + '----------------------------------------' + LF + CR;
         //MG_Right(FormatFloat('#,##0', nTaxResult), 15) + '원'
         sRPrt:= sRPrt + '신용카드 세부 내역' + LF + CR;
         sRPrt:= sRPrt + '총공급가액: ' + MG_Right(FormatFloat('#,##0', nTaxResult), 15) + '원' + LF + CR;
         sRPrt:= sRPrt + '총부가세액: ' + MG_Right(FormatFloat('#,##0', nTax), 15) + '원' + LF + CR;
         sRPrt:= sRPrt + ' 총청구액 : ' + MG_Right(FormatFloat('#,##0', nMoneyTotal), 15) + '원' + LF + CR;
      end;
