...
interface
uses
  Windows, SysUtils, Forms, WinInet, IniFiles, System.DateUtils;
...
const
  ...
  YOColor = AnsiChar($02) + AnsiChar($02) + AnsiChar($02) + AnsiChar($02) +
            ...
            AnsiChar($01) + AnsiChar($01) + AnsiChar($01) + AnsiChar($01) +
            AnsiChar($01) + AnsiChar($01) + AnsiChar($01) + AnsiChar($01);
  ReadInPutSignal = AnsiChar($02) + AnsiChar($FF) + AnsiChar($00) + AnsiChar($01) + AnsiChar($52)
                  + AnsiChar($00) + AnsiChar($00) + AnsiChar($00) + AnsiChar($AE) + AnsiChar($03) ;
...
var
  ...
  nHostPort, nDspInterval,nArmInterval: Word;

  nGCount: Integer = 0;
  nGCountInterval : Integer = 0;
  bCount: Boolean = False;     //카운트 체크
  ...
  bSaveION: Boolean = False;         //경차 관련하여 입차 데이터 저장 처리 여부 확인
  ...
  sTempFile1, sTempFile2, sTempFile3, sTempFile4, sTempFile5: aString;
  ...
  //세대통보용
  sHomeInfo_IP, sHomeInfo_Temp, sLocalIP, sRecvHomeServer: aString;
  nHomeInfo_Port: Cardinal;
  bHomeInfo: Boolean = False;

  //이지빌용 동,호 저장 변수...
  sezVilleDong, sezVilleHo, sHomeInfo_CarNo, sHomeInfo_Dong, sHomeInfo_Ho, sHomeInfo_ShortCarNo: aString;
  ...
  bFullCar : Boolean = False;       //주차가능 대수가 0일때(만차일때 : true, 정상 : false);
  {
