...
type
  TfrmSetup = class(TForm)
    ...
    btnExit: TButton;
    edt1_OP4: TLUCombo;
    edt4_home28: TAdvLUEdit;
    edt4_home29: TAdvLUEdit;
    edt3_Sp11: TLUCombo;
    edt3_Sp12: TLUCombo;
    edt1_OP14: TLUCombo;
    EdtEarthQuake: TLUCombo;
    ...
    edt4_home31: TLUCombo;
    edt4_home32: TLUCombo;
    edt4_home33: TLUCombo;
    edt2_Default12: TLUCombo;
    edt4_home34: TLUCombo;
    edt4_home35: TLUCombo;
    edt1_OP15: TLUCombo;
    edt1_OP16: TLUCombo;
    edt2_Default13: TLUCombo;
    edt2_Default14: TLUCombo;
    edt1_OP17: TLUCombo;
...
begin
  try
    ...
    WriteINI2(PARK,'입차LPR수',             edt1_LPR1.ItemIndex);
    WriteINI2(PARK,'출차LPR수',             edt1_LPR2.ItemIndex);
    WriteINI2(PARK,'아파트',                edt1_LPR3.ItemIndex);
    WriteINI2(PARK,'차단기 제어여부',       edt1_OP1.ItemIndex);
    WriteINI2(PARK,'차단기 제어',           edt1_OP2.ItemIndex);
    WriteINI2(PARK,'개방운영',              edt1_OP3.ItemIndex);
    WriteINI2(PARK,'LPR별개방운영사용',     edt1_OP4.ItemIndex);
    WriteINI2(PARK,'일반차량 출차자동개방', edt1_OP5.ItemIndex);
    WriteINI2(PARK,'정기차량기간만료 출차자동개방', edt1_OP14.ItemIndex);
    WriteINI2(PARK,'운영모드자동변경',      edt1_OP6.ItemIndex);
    WriteINI(PARK,'개방시작',              edt1_OP7.Text);
    WriteINI(PARK,'개방종료',              edt1_OP7.Text);
    WriteINI2(PARK,'MI_IN',                 edt1_OP9.ItemIndex);
    WriteINI2(PARK,'MI_IN_INOPEN',          edt1_OP15.ItemIndex);
    WriteINI2(PARK,'방문차량차단기',        edt1_OP17.ItemIndex);
    WriteINI2(PARK,'이미지 로드 시간',      edt1_OP16.ItemIndex);
    WriteINI2(PARK,'6Proc',                 edt1_OP10.ItemIndex);
    WriteINI2(PARK,'로그인',                edt1_OP11.ItemIndex);
    WriteINI2(PARK,'모니터링유무',          edt1_OP12.ItemIndex);
    WriteINI2(PARK,'mMirrorDsp',            edt1_OP13.ItemIndex);
    WriteINI2(PARK,'방문증',                edt1_Visit1.ItemIndex);
    ...
    WriteINI2(DSP,'정기문구사용',           edt2_Default5.ItemIndex           );
    WriteINI2(DSP,'DisplayParkingTimeUse',  edt2_Default12.ItemIndex          );

    WriteINI(DSP,'정기차량문구',          edt2_Default6.Text           );
    WriteINI(DSP,'일반차량문구',           edt2_Default7.Text           );
    ...
    WriteINI2(DSP,'입구보조전광판',        edt2_Default14.ItemIndex    );
    WriteINI2(DSP,'출구보조전광판',        edt2_Default13.ItemIndex    );
    ...
    //특수기능설정
    WriteINI2(PARK,'블랙리스트',                 edt3_Black1.ItemIndex     );
    WriteINI2(PARK,'블랙리스트의심사용',         edt3_Black2.ItemIndex             );
    WriteINI2(PARK,'블랙리스트소리사용',         edt3_Black3.ItemIndex             );
    WriteINI(PARK,'블랙리스트의심입차회수',     edt3_Black4.Text                 );
    WriteINI(PARK,'Alarm File',                 edt3_Black5.Text     );
    WriteINI(PARK,'블랙리스트의심창유지시간',   edt3_Black6.Text                   );
    ...
    WriteINI2(PARK,'부제처리사용',               edt3_Sp6.ItemIndex       );
    WriteINI2(PARK,'부제처리종류',               edt3_Sp7.ItemIndex       );
    WriteINI2(PARK,'31일부제처리',               edt3_Sp8.ItemIndex       );
    WriteINI(PARK,'WestInLprUnitNo',            edt3_Sp9.Text          );
    WriteINI2(PARK,'차량번호 4자리 인식여부',    edt3_Sp10.ItemIndex                  );
    WriteINI2(PARK,'단기정기권사용',    edt3_Sp11.ItemIndex                  );
    WriteINI2(PARK,'CameraCHInout',    edt3_Sp12.ItemIndex                  );

    //홈넷설정
    WriteINI2(PARK,'홈넷',                        edt4_home1.ItemIndex);
    WriteINI2(PARK,'홈넷버전',                    edt4_home2.ItemIndex  );
    ...
    WriteINI2(PARK,'홈넷방문자',                  edt4_home8.ItemIndex    );
    WriteINI2(PARK,'홈넷포인트사용',              edt4_home32.ItemIndex    );
    WriteINI2(PARK,'홈넷사전방문제한',            edt4_home34.ItemIndex    );
    WriteINI2(PARK,'홈넷2',                       edt4_home35.ItemIndex    );
    ...
    WriteINI2(PARK,'smartVillageVisit_Ver',      edt4_home33.ItemIndex        );
    WriteINI2(PARK,'smartVillageVisit',           edt4_home30.ItemIndex        );
    WriteINI2(PARK,'smartVillagePoint',           edt4_home31.ItemIndex        );
    ...
    WriteINI(PARK,'HomeInfo IP2',                   edt4_home28.Text   );
    WriteINI(PARK,'HomeInfo Port2',                 edt4_home29.Text     );
    ...
    //아크로포레스트(지진)
    WriteINI2(PARK,'지진연동',                EdtEarthQuake.ItemIndex);

    bSetupChange := True;
    MessageBoxTimeOut(Application.Handle, '저장성공'+#13#10+'(1초뒤 화면이 닫힙니다.)', '', 0, 0, 1000);
  except on E: Exception do
  begin
    bSetupChange := False;
    MessageBoxTimeOut(Application.Handle, '저장실패!'+#13#10+'(2초뒤 화면이 닫힙니다.)', '', 0, 0, 2000);
  end;
  end;
end;
...
begin
  ...
  edt1_LPR1.ItemIndex := ReadINI(PARK,'입차LPR수','');
  edt1_LPR2.ItemIndex := ReadINI(PARK,'출차LPR수','');
  edt1_LPR3.ItemIndex := ReadINI(PARK,'아파트','');

  edt1_OP1.ItemIndex := ReadINI(PARK,'차단기 제어여부','');
  edt1_OP2.ItemIndex := ReadINI(PARK,'차단기 제어','');
  edt1_OP3.ItemIndex := ReadINI(PARK,'개방운영','');
  edt1_OP4.ItemIndex := ReadINI(PARK,'LPR별개방운영사용','');
  edt1_OP5.ItemIndex := ReadINI(PARK,'일반차량 출차자동개방','');
  edt1_OP14.ItemIndex := ReadINI(PARK,'정기차량기간만료 출차자동개방','');
  edt1_OP6.ItemIndex := ReadINI(PARK,'운영모드자동변경','');
  edt1_OP7.Text := ReadINI(PARK,'개방시작','');
  edt1_OP8.Text := ReadINI(PARK,'개방종료','');
  edt1_OP9.ItemIndex := ReadINI(PARK,'MI_IN','');
  edt1_OP15.ItemIndex := ReadINI(PARK,'MI_IN_INOPEN','');
  edt1_OP17.ItemIndex := ReadINI(PARK,'방문차량차단기','');
  edt1_OP16.ItemIndex := ReadINI(PARK,'이미지 로드 시간','');
  edt1_OP10.ItemIndex := ReadINI(PARK,'6Proc','');
  edt1_OP11.ItemIndex := ReadINI(PARK,'로그인','');
  edt1_OP12.ItemIndex := ReadINI(PARK,'모니터링유무','');
  edt1_OP13.ItemIndex := ReadINI(PARK,'mMirrorDsp','');

  edt1_Visit1.ItemIndex := ReadINI(PARK,'방문증','');
  ...
  edt2_Default5.ItemIndex := ReadINI(DSP,'정기문구사용','');
  edt2_Default12.ItemIndex := ReadINI(DSP,'DisplayParkingTimeUse','');
  edt2_Default6.Text := ReadINI(DSP,'정기차량문구','',1);
  edt2_Default7.Text := ReadINI(DSP,'일반차량문구','',1);
  ...
  edt2_Default14.ItemIndex := ReadINI(DSP,'입구보조전광판','');
  edt2_Default13.ItemIndex := ReadINI(DSP,'출구보조전광판','');
  ...
  //특수기능설정
  edt3_Black1.ItemIndex := ReadINI(PARK,'블랙리스트','');
  edt3_Black2.ItemIndex := ReadINI(PARK,'블랙리스트의심사용','');
  edt3_Black3.ItemIndex := ReadINI(PARK,'블랙리스트소리사용','');
  edt3_Black4.Text := ReadINI(PARK,'블랙리스트의심입차회수','');
  edt3_Black5.Text := ReadINI(PARK,'Alarm File','');
  edt3_Black6.Text := ReadINI(PARK,'블랙리스트의심창유지시간','');
  ...
  edt3_Sp6.ItemIndex := ReadINI(PARK,'부제처리사용','');
  edt3_Sp7.ItemIndex := ReadINI(PARK,'부제처리종류','');
  edt3_Sp8.ItemIndex := ReadINI(PARK,'31일부제처리','');
  edt3_Sp9.Text := ReadINI(PARK,'WestInLprUnitNo','');
  edt3_Sp10.ItemIndex := ReadINI(PARK,'차량번호 4자리 인식여부','');
  edt3_Sp11.ItemIndex := ReadINI(PARK,'단기정기권사용','');
  edt3_Sp12.ItemIndex := ReadINI(PARK,'CameraCHInout','');

  //홈넷설정
  edt4_home1.ItemIndex := ReadINI(PARK,'홈넷','');
  edt4_home2.ItemIndex := ReadINI(PARK,'홈넷버전','');
  edt4_home32.ItemIndex := ReadINI(PARK,'홈넷포인트사용','');
  edt4_home34.ItemIndex := ReadINI(PARK,'홈넷사전방문제한','');
  ...
  edt4_home8.ItemIndex := ReadINI(PARK,'홈넷방문자','');
  edt4_home35.ItemIndex := ReadINI(PARK,'홈넷2','');
  ...
  edt4_home33.ItemIndex := ReadINI(PARK,'smartVillageVisit_Ver','');
  edt4_home30.ItemIndex := ReadINI(PARK,'smartVillageVisit','');
  edt4_home31.ItemIndex := ReadINI(PARK,'smartVillagePoint','');
  ...
  edt4_home28.Text := ReadINI(PARK,'HomeInfo IP2','');
  edt4_home29.Text := ReadINI(PARK,'HomeInfo Port2','');
  ...
  //지진연동(아크로포레스트)
  edtEarthQuake.ItemIndex := ReadINI(PARK,'지진연동','');
