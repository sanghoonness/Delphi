...
type
  TfrmIONData = class(TForm)
    ...
    procedure btnIONCountClick(Sender: TObject);
  private
...
begin
  try
    ...
    begin
      ...
      begin
        if cmbType.ItemIndex = 0 then
          //SQL.Add('Select * from IONData where ProcDate >= :N1 and ProcTime >= :N2 and ProcDate <= :N6 and ProcTime <= :N3 and ')
          SQL.Add('Select * from IONData where (ProcDate +'' ''+ProcTime >= :N1 and ProcDate +'' ''+ProcTime <=  :N2) and ')
        else
        if cmbType.ItemIndex = 1 then
          //SQL.Add('Select * from IONData where OutDate >= :N1 and OutTime >= :N2 and OutDate <= :N6 and OutTime <= :N3 and ');
          SQL.Add('Select * from IONData where (OutDate +'' ''+OutTime >= :N1 and OutDate +'' ''+OutTime <=  :N2) and ');
    ...
    begin
      ...
      SQL.Add('Select Count(*) nIn from IONData where (ProcDate +'' ''+ProcTime >= :N1 and ProcDate +'' ''+ProcTime <=  :N2) and ');
      SQL.Add('(TKType = :N3 or TKType = :N5) and Status = :N4 ');
...
//      Parameters.ParamByName('N5').Value:= 9;
      ...
      ExceptLogging(inttostr(nNIn));

      Close;
      SQL.Clear;
      SQL.Add('Select Count(*) nOut from IONData where (ProcDate +'' ''+ProcTime >= :N1 and ProcDate +'' ''+ProcTime <=  :N2) and ');
      SQL.Add('(TKType = :N3 or TKType = :N6) and Status = :N4 and OutChk > :N5 ');
...
begin
  try
    ...
    begin
      ...
      dg1.DataSource.DataSet.RecNo := dg1.Row;
      ...
      begin
        Close;
        SQL.Clear;
        SQL.Add('Select * from IONData where ProcDate = :N1 and ProcTime = :N2 ');
        Parameters.ParamByName('N1').Value:= dg1.Cells[2, dg1.Row];
        Parameters.ParamByName('N2').Value:= dg1.Cells[3, dg1.Row];
        Open;
...
begin
  //mzExcelExport(dg1,dg1.ColCount-1);
  try
    bExcelSaving := True;
...
procedure TfrmIONData.btnIONCountClick(Sender: TObject);
begin
  NextModalDialog(TfrmIONCount.Create(Self));
end;
