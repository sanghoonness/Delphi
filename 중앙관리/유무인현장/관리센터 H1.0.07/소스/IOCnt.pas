...
begin
 ...
 //   Parameters.ParamByName('N3').Value := nCurrParkNo;
    ...
    SQL.Add('select count(*) nLprCnt from (select UnitNo UnitNo from IONData where UnitNo<>0 and ParkNo = :N1 group by UnitNo    ');
    SQL.Add('union all ');
    SQL.Add('select OutUnitNo UnitNo from IONData where OutUnitNo<>0 and ParkNo = :N2 group by OutUnitNo) F ');
 ...
 //   SQL.Add('From UnitInfo Where UnitKind = :N1 and ParkNo = :N2 Order By ParkNo, UnitNo');
    SQL.Add('select A.UnitNo UnitNo,Max(B.UnitName) UnitName From IONData A ,UnitInfo B where A.UnitNo<>0 and A.UnitNo=B.UnitNo and A.ParkNo=:N1 group by A.UnitNo  ');
//    Parameters.ParamByName('N1').Value := 8; // 입구LPR
    Parameters.ParamByName('N1').Value := nCurrParkNo;
    Open;
    i:= 1;
 ...
 //   SQL.Add('Select * ');
 //   SQL.Add('From UnitInfo Where UnitKind = :N1 and ParkNo = :N2 Order By ParkNo, UnitNo');
    SQL.Add('select OutUnitNo UnitNo,Max(B.UnitName) UnitName From IONData A ,UnitInfo B where A.OutUnitNo<>0 and A.OutUnitNo=B.UnitNo and A.ParkNo=:N1 group by A.OutUnitNo ') ;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        SQL.Add('OutUnitNo = :N3 and OutTime >= :N4 and OutTime <= :N5 and OutChk > :N6 ');
