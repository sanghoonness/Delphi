...
//            dmTables.ADODB.RollbackTrans;
//            ExceptLogging('TfrmCustInfo.btnAllAddClick: ' + aString(E.Message));
//            ShowMessage('정기차량 개인정보등록중 오류발생! 관리자에 문의바랍니다.몇번쨰'+aString(i));
//          end;
//        end;
//
//        try
...
//          Parameters.ParamByName('pCompName').Value:= sCompName;  // 동, 호 (회사명, 부서명)
//          Parameters.ParamByName('pDeptName').Value:= sDeptName;
//          Parameters.ParamByName('pName').Value:= sName;          // 성명
//          Parameters.ParamByName('pTelNo').Value:= sTelNo;        // 전화번호
...
//            dmTables.ADODB.RollbackTrans;
//            ExceptLogging('TfrmCustInfo.btnAllAddClick: ' + aString(E.Message));
//            ShowMessage('정기차량 발급정보등록중 오류발생! 관리자에 문의바랍니다..몇번쨰'+aString(i));
//          end;
//        end;
//        nTotCnt:= nTotCnt + 1;
//      end
...
//              dmTables.ADODB.RollbackTrans;
//              ExceptLogging('TfrmCustInfo.btnAllAddClick: ' + aString(E.Message));
//              ShowMessage('정기차량 개인정보갱신중 오류발생! 관리자에 문의바랍니다..몇번쨰'+aString(i));
//            end;
//          end;
//
//          try
  ...
  try
    ...
    begin
      try
        ...
        Parameters.ParamByName('N8' ).Value:= 6;  // 삭제 IssueType = 6
    ...
    ExceptLogging('차량삭제: ' + edtCarNo.Text);
    frmClear;
  except
             ...
             begin
               try
                 ...
                 begin
                   ...
                   Parameters.ParamByname('N7' ).Value:= 2;
                   Parameters.ParamByName('N8' ).Value:= 2;   // 기간연장
                   Parameters.ParamByName('N9' ).Value:= edtCurrAmt.Value;
                   Parameters.ParamByName('N10').Value:= sSDate;
                   Parameters.ParamByName('N11').Value:= sEDate;
                   Parameters.ParamByName('N12').Value:= edtIssueAmt.Value ;
                   ...
                   Parameters.ParamByName('pPayType').Value:= cmbPayType.ItemIndex;
                   ExecSql;
                   ExceptLogging('차량번호변경 : ' + sPrvCarNo + ' -> ' + edtCarNo.Text);
                 end
                 else
                 begin
                    // 차량번호 변경
                    ...
                    Parameters.ParamByName('N6' ).Value:= edtTKNo.Text;
                    Parameters.ParamByname('N7' ).Value:= 2;
                    Parameters.ParamByName('N8' ).Value:= 5;    // 정보변경
                    Parameters.ParamByName('N9' ).Value:= edtCurrAmt.Value;
                    Parameters.ParamByName('N10').Value:= sSDate;
                    Parameters.ParamByName('N11').Value:= sEDate;
                    Parameters.ParamByName('N12').Value:= edtIssueAmt.Value ;
                    ...
                    Parameters.ParamByName('pGroupNo').Value:= LongInt(cmbGroupNo.Items.Objects[cmbGroupNo.ItemIndex]);
                    Parameters.ParamByName('pPayType').Value:= cmbPayType.ItemIndex;
                    ExceptLogging('기간변경 : (' + sPrvCarNo + ') ' + sPrvExpDateF + ' -> ' + sSDate);
                    ExceptLogging('기간변경 : (' + sPrvCarNo + ') ' + sPrvExpDateT + ' -> ' + sEDate);

                   if (sPrvCurrAmt <> IntToStr(edtCurrAmt.Value)) then
                    ExceptLogging('월정금액변경 : (' + sPrvCarNo + ') ' + sPrvCurrAmt + ' -> ' + IntToStr(edtCurrAmt.Value));
  ...
  try
    ...
    begin
      ...
      begin
        ...
        edtCurrAmt.Value:= FieldByName('CurrAmt').AsInteger;        // 월정금액
...
begin
    ...
    if Day=dateDaysInMonth(CurrentDate) then  // 마지막일이면
    begin
