...
uses
  ...
  AdvToolBtn, AdvAppStyler, Buttons, tmsAdvGridExcel, AdvObj, AdvDateTimePicker;
    ...
    begin
      case cmbType.ItemIndex of
       // 1: ShowMessage('차량번호를 입력하여주세요');
        ...
        3: ShowMessage('호수을 입력하여주세요');
        4: ShowMessage('휴대폰번호를 입력하여주세요');
      end;
      edtSeek.SetFocus;
      Exit;
    end;
    ...
    begin
      ...
      begin
        case cmbType.ItemIndex of
          ...
          3: begin
               SQL.Add('and DeptName like ' + MG_MakeStr('%' + Trim(edtSeek.Text) + '%'));
               SQL.Add(' Order By DeptName');
          end;
          4: begin
               SQL.Add('and TelNo like ' + MG_MakeStr('%' + Trim(edtSeek.Text) + '%'));
               SQL.Add(' Order By TelNo');
          end;
        end;
      end
      else
...
begin
  ...
  if nApt = 1 then
  begin
    DBAdvGrid1.Columns[4].Header := '동';
    DBAdvGrid1.Columns[5].Header := '호';
  end;
...
begin
  try
    sDate:= FormatDateTime('YYYY-MM-DD', edtDate.Date);
    qryGroup.Active:= True;
    qryIOStatus.Active:= True;
