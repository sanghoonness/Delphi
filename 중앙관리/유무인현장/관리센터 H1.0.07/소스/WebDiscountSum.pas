  ...
  try
    ...
    begin
      ...
      if cmbMode.ItemIndex = 0 then
        ...
        SQL.Add(' where DelYn=0 and outymd is not null ');// 출차기준
      ...
      begin
        ...
        while not Eof do
        begin
          sgWebDC.Cells[0,tmpCnt] := FieldByName('StoreName').AsString;       // 상점명
          sgWebDC.Cells[1,tmpCnt] := FieldByName('DiscountName').AsString;    // 할인종류
          sgWebDC.Cells[2,tmpCnt] := FieldByName('DiscountCount').AsString;   // 할인횟수
          sgWebDC.Cells[3,tmpCnt] := FieldByName('Reserve7').AsString;        // 할인금액
          sgWebDC.Cells[4,tmpCnt] := FieldByName('Reserve6').AsString;        // 청구금액
   ...
   begin
      ...
      begin             
        ...
        sgWebDC.Cells[2, sgWebDC.RowCount] := '총 건수: ' + FormatFloat('#,##0', FieldByName('cnt').AsInteger) + '건' ;
        sgWebDC.Cells[3, sgWebDC.RowCount] := FormatFloat('#,##0', FieldByName('amt2').AsInteger) + '원';
        sgWebDC.Cells[4, sgWebDC.RowCount] := FormatFloat('#,##0', FieldByName('amt1').AsInteger) + '원';
        ...
        edtTotal.Caption := ' 총 건수: ' + FormatFloat('#,##0', FieldByName('cnt').AsInteger)
                          + '건, 총 할인금액: ' + FormatFloat('#,##0', FieldByName('amt2').AsInteger)  + '원'
                          + ', 총 청구금액: ' + FormatFloat('#,##0', FieldByName('amt1').AsInteger)  + '원';
      end
      else
        edtTotal.Caption := ' 총 건수: 0건, 총 할인금액: 0건, 총 청구금액: 0원';
    end;
...
begin
  ...
  begin
    ColCount := 5;
    RowCount := 2;
    ColWidths[0] :=  100;     Cells[0, 0] := '상점명';      Alignments[0, 0] := taCenter;
    ColWidths[1] :=  130;    Cells[1, 0] := '할인종류';      Alignments[1, 0] := taCenter;
    ColWidths[2] :=  130;    Cells[2, 0] := '할인횟수';      Alignments[2, 0] := taCenter;
    ColWidths[3] :=  100;    Cells[3, 0] := '할인금액';      Alignments[3, 0] := taCenter;
    ColWidths[4] :=  100;    Cells[4, 0] := '청구금액';  Alignments[4, 0] := taCenter;
  end;
end;
