...
uses
  ...
  Dialogs, DB, ADODB, StdCtrls, ExtCtrls, Spin, Mask, Buttons, ComCtrls;

type
  TfrmAddRFSC = class(TForm)
    sb1: TStatusBar;
    btnSave: TBitBtn;
    btnCancel: TBitBtn;
    btnClose: TBitBtn;
...
procedure TfrmAddRFSC.btnCancelClick(Sender: TObject);
begin
  sSCChk:= '';
  edtMarkNo.Text:= '';
  edtTKNo.Text:= '';
...
procedure TfrmAddRFSC.btnCloseClick(Sender: TObject);
begin
   ModalResult := mrCancel;
  //Close;
end;

procedure TfrmAddRFSC.btnSaveClick(Sender: TObject);
var
  sData, sSend: String;
  i: Integer;
begin
  try
    ...
    begin
      ...
      begin
        ShowMessage('해당 RF정기권에 대한 자료가 없습니다. 확인하여주세요!');
        edtMarkNo.Text:= '';
        edtTKNo.Text:= '';
        edtCarNo.SetFocus;
        Exit;
      ...
      SQL.Add('CarNo = :N6, CompName = :N7, DeptName = :N8, Address = :N9, WPNo = :N14, IssueAmt = :N15 ');
      SQL.Add('Where ParkNo = :N10 and TKType = :N11 and GroupNo = :N12 and TKNo = :N13');
      Parameters.ParamByName('N1' ).Value:= edtExpDateF.Text;
      Parameters.ParamByName('N2' ).Value:= edtExpDateT.Text;
      ...
      Parameters.ParamByName('N11').Value:= 99;
      Parameters.ParamByname('N12').Value:= StrToInt(cmbGroupNo.Text);
    ...
    on E: Exception do ExceptLogging('TfrmAddRFSC.btnSaveClick: ' + E.Message);
  end;
end;

procedure TfrmAddRFSC.btnSeekClick(Sender: TObject);
begin
...
procedure TfrmAddRFSC.edtSeekKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmAddRFSC.FormShow(Sender: TObject);
var
  i: Byte;
begin
  try
    ...
    edtExpDateT.Text:= MG_AddDate(FormatDateTime('YYYYMMDD', Now), 30);
    edtCarNo.SetFocus;
  except
    on E: Exception do ExceptLogging('TfrmAddRFSC.FormShow: ' + E.Message);
  end;
end;
