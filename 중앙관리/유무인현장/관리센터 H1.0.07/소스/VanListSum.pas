...
begin
  ...
  begin
   ...
   ColCount := 19;
    ...
    ColWidths[17] :=  50;    Cells[17, 0] := 'NH카드';       Alignments[17, 0] := taCenter;
  ...
  sgVanList.MergeCells(17, 0, 2, 1); //  'NH카드
end;
...
begin
  try
    ...
    begin
      ...
      SQL.Add(', isnull(totalCnt8, 0) as totalCnt8, isnull(totalAmt8, 0) as totalAmt8  ');     //NH카드
      SQL.Add('from(  ');
      ...
      SQL.Add('from VanList ');
      SQL.Add('where CardType = ''NH카드'' ');     // NH카드
      SQL.Add('group by ProcDate, CardType ');
      SQL.Add(') H ');
      ...
      begin
        ...
        begin
         ...
         // sgVanList.Cells[1,tmpCnt] := FormatFloat('#,##0', StrToInt(FieldByName('totalCnt').AsString));  // 총수량
          ...
          sgVanList.Cells[17,tmpCnt] := FormatFloat('#,##0', StrToInt(FieldByName('totalCnt8').AsString));  // NH카드
          ...
          nTotalCnt8 := nTotalCnt8 + FieldByName('totalCnt8').AsInteger;     // NH카드
          nTotalAmt8 := nTotalAmt8 + FieldByName('totalAmt8').AsInteger;     // NH카드
        ...
        sgVanList.Cells[17,tmpCnt] := FormatFloat('#,##0', nTotalCnt8);  // NH카드
